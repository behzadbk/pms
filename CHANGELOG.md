...
## اصلاحات پنل مدیر ساختمان + نقش حسابداری (۲ مهر ۱۴۰۵) — جزئیات: docs/05-ADMIN-PANEL-FIXES.md

- قوانین رزرو هوشمند: افزودن، ویرایش و حذف مشاع، قوانین و سانس‌ها (قبلاً فقط‌خواندنی بود و آیکون ویرایش کار نمی‌کرد).
- رزرو مشاعات: ثبت دستی رزرو برای هماهنگی تلفنی + «عدم تایید» با دلیل که برای ساکن اعلان می‌شود.
- اعلانات و رأی‌گیری: فرم اعلان و فرم‌ساز نظرسنجی (چند سؤال و گزینه)، انتخاب مخاطب (نقش‌ها، بلوک‌ها، واحدها)، اعلان داخل برنامه با زنگوله‌ی واقعی، صفحه‌ی اعلانات و رأی‌دادن برای ساکن، پرسنل، نگهبانی و حسابداری.
- تیکت‌ها: نمایش جزئیات، تشخیص هوشمند تجهیز (مثلاً آسانسور B)، نمایش آخرین سرویس/تعویض و سابقه‌ی کامل، هشدار موعد سرویس، ثبت تعمیر/تعویض در سابقه‌ی تجهیز.
- نقش جدید «حسابداری» (`accountant`): شارژ و مطالبات و ثبت فاکتور به پنل حسابداری منتقل شد. مدیر فقط گزارش مالی و جزئیات فاکتورها را می‌بیند. `generate-monthly` → `@Roles('accountant')`. مایگریشن `003_accountant_role.sql` و کاربر seed `accountant@borj-aftab.test`.
