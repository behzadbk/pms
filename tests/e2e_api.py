...
# ── ۱.۱ کارکنان: ورود با نام کاربری + مدیریت توسط مدیر (مایگریشن 004)
s,b=call("identity","POST","/auth/login",{"email":"kitchen","password":"Passw0rd!","tenantSubdomain":T})
check("staff","ورود کارمند با نام کاربری",s in(200,201) and b["user"]["role"]=="staff" and set(b["user"].get("permissions",[]))=={"kitchen","cafe"},(s,b))
s,b=call("identity","GET","/users/staff",token=tok["staff"]); check("staff","لیست کارکنان برای غیرمدیر → 403",s==403,s)
s,b=call("identity","POST","/users/staff",{"fullName":"کارمند تست","username":"e2e.lobby","password":"secret12","department":"lobby","permissions":["cafe"]},token=tok["admin"])
check("staff","ساخت کارمند توسط مدیر",s in(200,201) and set(b.get("effectivePermissions",[]))=={"lobby","cafe"},(s,b)); SID=b.get("id")
s,b=call("identity","POST","/users/staff",{"fullName":"x2","username":"e2e.lobby","password":"secret12","department":"lobby"},token=tok["admin"]); check("staff","نام کاربری تکراری → 409",s==409,s)
s,b=call("identity","POST","/users/staff",{"fullName":"x3","username":"e2e.bad","password":"secret12","department":"ceo"},token=tok["admin"]); check("staff","بخش نامعتبر → 400",s==400,s)
s,b=call("identity","POST","/auth/login",{"email":"e2e.lobby","password":"secret12","tenantSubdomain":T}); check("staff","ورود کارمند جدید",s in(200,201),(s,b))
if SID:
    s,b=call("identity","PATCH",f"/users/staff/{SID}",{"isActive":False},token=tok["admin"]); check("staff","غیرفعال‌سازی",s==200 and b.get("isActive") is False,(s,b))
    s,b=call("identity","POST","/auth/login",{"email":"e2e.lobby","password":"secret12","tenantSubdomain":T}); check("staff","کارمند غیرفعال نمی‌تواند وارد شود → 401",s==401,s)
    s,b=call("identity","PATCH","/users/staff/22222222-2222-2222-2222-222222222221",{"fullName":"hack"},token=tok["admin"]); check("staff","API کارکنان به حساب مدیر دست نمی‌زند → 404",s==404,s)
    s,b=call("identity","DELETE",f"/users/staff/{SID}",token=tok["admin"]); check("staff","حذف کارمند",s==200,s)
