...
interface UnitServiceClient {
  ...
  listUnitsByTenant(data: { tenantId: string }): Observable<{ units: UnitReply[] }>
}
...
export class PropertyClientService implements OnModuleInit {
  ...
  async listUnitsByTenant(tenantId: string): Promise<UnitReply[]> {
    const res = await firstValueFrom(this.unitService.listUnitsByTenant({ tenantId }))
    return res.units
  }
}
